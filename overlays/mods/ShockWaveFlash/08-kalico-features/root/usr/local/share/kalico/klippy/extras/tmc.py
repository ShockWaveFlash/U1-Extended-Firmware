    ...
    def _query_register(self, reg_info, try_clear=False):
        ...
        while True:
            ...
            if count >= 3:
                fmt = self.fields.pretty_format(reg_name, val)
                err_msg = (
                    '{"coded": "0003-0522-0000-0011", "oneshot": 0, '
                    '"msg":"%s"}'
...
class TMCVirtualPinHelper:
    def __init__(self, config, mcu_tmc):
        ...
        # Dynamic set homing tcoolthrs (Snapmaker)
        self.config = config
        self.name = config.get_name().split()[-1]
        self.dynamic_homing_coolthrs = config.getboolean(
            "dynamic_homing_coolthrs", True
        )
        self.enable_sim_stall_set_endstops = config.getboolean(
            "enable_sim_stall_set_endstops", True
        )
        self.dynamic_coolthrs_ratio = config.getfloat(
            "dynamic_coolthrs_ratio", 1.2, minval=0.0
        )
        self.homing_tcoolthrs = config.getint(
            "homing_tcoolthrs", None, minval=0
        )
    ...
    def handle_homing_move_begin(self, hmove):
        ...
        if reg is None:
            ...
            self._set_field(self.diag_pin_field, 1)
        # Enable tcoolthrs (Snapmaker: dynamisch aus der Homing-Geschwindigkeit)
        if self.dynamic_homing_coolthrs:
            if self.homing_tcoolthrs is not None:
                self._set_field(
                    "tcoolthrs", max(0, min(0xFFFFF, self.homing_tcoolthrs))
                )
            else:
                dynamic_tc_val = TMCtstepHelper(
                    self.mcu_tmc, hmove.homing_speed, config=self.config
                )
                final_tc_val = int(dynamic_tc_val * self.dynamic_coolthrs_ratio)
                final_tc_val = max(0, min(0xFFFFF, final_tc_val))
                self._set_field("tcoolthrs", final_tc_val)
        elif self.fields.get_field("tcoolthrs") == 0:
    ...
    def handle_homing_move_end(self, hmove):
        if (self.mcu_endstop not in hmove.get_mcu_endstops() and
            ...
            self.mcu_endstop not in hmove.get_mcu_sim_stall_set_endstops())):
            return
        # Restore previous state (tpwmthrs/en_pwm_mode/tcoolthrs/thigh)
...
class BaseTMCCurrentHelper:
    def __init__(self, config, mcu_tmc, max_current, has_sense_resistor=True):
        ...
        if has_sense_resistor:
            # Merge-Anpassung: Default 0.110 wiederhergestellt (Stock und
            # Mainline); [tmc2209 stepper_z] der U1-Config setzt ihn nicht.
