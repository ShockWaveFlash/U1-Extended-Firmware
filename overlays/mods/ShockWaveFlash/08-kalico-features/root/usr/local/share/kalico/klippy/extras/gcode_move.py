...
class GCodeMove:
    def __init__(self, config):
        ...
        self.speed = 25.0
        self.speed_factor = self.speed_factor_bak = 1.0 / 60.0
        ...
        wh.register_endpoint(
            "control/print_speed", self.handle_control_print_speed
        )
    ...
    def cmd_G1(self, gcmd):
            ...
            for axis, pos in self.axis_map.items():
                if axis in params:
                    ...
                    if not absolute_coord:
                        ...
                        if axis == "E":
                            delta_e = v - self.last_position[pos]
                        self.last_position[pos] = v + self.base_position[pos]
            if "F" in params:
                gcode_speed = float(params["F"])
                if gcode_speed <= 0.0:
                    error = '{"coded": "0003-0529-0000-0013", "msg":"%s"}' % (
                        "Invalid speed in '{}'".format(gcmd.get_commandline())
                    )
                    raise gcmd.error(error)
                self.speed = gcode_speed * self.speed_factor
        except ValueError as e:
            error = '{"coded": "0003-0529-0000-0014", "msg":"%s"}' % (
        ...
        if (
            ...
            and print_stats.state == "printing"
            and exclude_obj is not None
            and exclude_obj.get_status()["in_excluded_region"] == False
        ):
    ...
    def cmd_G20(self, gcmd):
        # Set units to inches
        error = '{"coded": "0003-0529-0000-0015", "msg":"%s"}' % (
