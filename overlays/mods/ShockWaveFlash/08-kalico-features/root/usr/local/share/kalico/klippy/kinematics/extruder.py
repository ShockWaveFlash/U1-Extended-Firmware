...
import chelper, coded_exception, queuefile, stepper
...
class ExtruderStepper:
    ...
    def __init__(self, config):
        ...
        # Snapmaker's flow_calibrator reads the configured smooth time
        self.config_smooth_time = self.smoother.smooth_time
        # No longer evaluated (dropped on the C side by both branches), kept
        # so that existing configs naming it still load
    ...
    # Snapmaker's flow_calibrator and virtual_sdcard read these directly
    @property
    def pressure_advance(self):
        return getattr(self.pa_model, "pressure_advance", 0.0)

    @property
    def pressure_advance_smooth_time(self):
        return self.smoother.smooth_time
    ...
    def _set_pressure_advance(self, pressure_advance, smooth_time):
        # Compatibility entry point for Snapmaker's flow_calibrator, which
        # sets a linear pressure advance directly
        pa_model = self.pa_model
        if pa_model.name != PALinearModel.name:
            pa_model = self.pa_models[PALinearModel.name]()
        pa_model.pressure_advance = pressure_advance
        self.smoother.smooth_time = smooth_time
        self._update_pressure_advance(
            pa_model, self.pressure_advance_time_offset
        )
...
class PrinterExtruder:
    def __init__(self, config, extruder_num):
        ...
        # Per-axis position in the (now 3-dimensional) extruder trapq
        self.extr_pos = [0.0, 0.0, 0.0]
        self.reactor = self.printer.get_reactor()
        ...
        need_save = False
        if self.nozzle_config_info['diameter'] != self.nozzle_config_info['diameter_v160']:
            self.nozzle_config_info['diameter_v160'] = self.nozzle_config_info['diameter']
            self.nozzle_config_info['volume_type'] = NOZZLE_CONFIG_DEFAULT['volume_type']
            need_save = True
        if 'version' not in self.nozzle_config_info:
            self.nozzle_config_info.update({'version': self.printer.get_start_args().get('software_version')})
            need_save = True

        if need_save:
            if not self.printer.update_snapmaker_config_file(self.nozzle_config_path, self.nozzle_config_info):
                logging.error("failed to sync nozzle config after lower version change")
        ...
        if (
            ...
            self.link_extruder_stepper(self.extruder_stepper)
    ...
    def link_extruder_stepper(self, extruder_stepper):
        if extruder_stepper not in self.extruder_steppers:
            ...
            extruder_stepper.stepper.set_position(self.extr_pos)
    ...
    def check_move(self, move, ea_index):
        ...
        if (not move.axes_d[0] and not move.axes_d[1]) or axis_r < 0.:
            ...
            if abs(axis_d) > self.max_e_dist:
                ...
                msg = "Extrude only move too long (%.3fmm vs %.3fmm), See the 'max_extrude_only_distance' config option for details" % (axis_d, self.max_e_dist)
                coded_msg = '{"coded":"%s", "msg":"%s"}' % (coded, msg)
                raise self.printer.command_error(coded_msg)
    ...
    # ea_index defaults to 3 so the classic single-extruder callers still work
    def calc_junction(self, prev_move, move, ea_index=3):
    ...
    def process_move(self, print_time, move, ea_index):
        ...
        extr_pos = self.extr_pos
        if move.is_kinematic_move:
            # Regular kinematic move with extrusion
            extr_r = [math.copysign(r * r, axis_r) for r in move.axes_r[:3]]
        elif self.is_calibrating_flow:
            # Snapmaker flow calibration extrudes without any XY motion, but
            # pressure advance must stay active.  kin_extruder.c enables
            # pressure advance for axes_r.x > 0, so encode the calibration
            # move like a kinematic move along x.
            extr_r = [axis_r, 0.0, 0.0]
        else:
        ...
        for i in range(3):
            self.extr_pos[i] += extr_d * extr_r[i]
        self.last_position = move.end_pos[ea_index]

    # Snapmaker's toolhead calls extruder.move(print_time, move); Kalico
    # renamed it to process_move() with an explicit axis index
    def move(self, print_time, move):
        self.process_move(print_time, move, 3)
    ...
    def _set_nozzle_diameter(self, diameter, save=True):
        ...
        if diameter >= 0.1999 and diameter <= 0.2001:
            ...
            if print_task_config is not None and filament_parameters is not None:
                status = print_task_config.get_status()
                is_allow = filament_parameters.is_allow_to_print(
                ...
                if not is_allow:
                    print_task_config.reset_filament_info(self.extruder_index)
    ...
    def _handle_control_nozzle_diameter(self, web_request):
        try:
            ...
            extruder_obj.nozzle_config_info['version'] = self.printer.get_start_args().get('software_version')
    ...
    def _handle_control_nozzle_properties(self, web_request):
        try:
            logging.info(f"[extruder] wb, control_nozzle_properties: {web_request.get_raw_parameters()}")
            ...
            print_stats = self.printer.lookup_object('print_stats', None)
            if print_stats is not None and print_stats.state in ['printing', 'paused']:
                raise ValueError("Cannot change nozzle properties during printing!")
            ...
            if nozzle_volume_type is not None:
                if nozzle_volume_type not in VALID_NOZZLE_VOLUME_TYPES:
                    raise ValueError(f"nozzle_volume_type error: {nozzle_volume_type}")
                extruder_obj._set_nozzle_volume_type(nozzle_volume_type, save=False)

            extruder_obj.nozzle_config_info['version'] = self.printer.get_start_args().get('software_version')

            if not extruder_obj.printer.update_snapmaker_config_file(extruder_obj.nozzle_config_path, extruder_obj.nozzle_config_info):
    ...
    def cmd_SET_NOZZLE_PROPERTIES(self, gcmd):
        logging.info("[extruder] SET_NOZZLE_PROPERTIES %s", gcmd.get_raw_command_parameters())
        ...
        if volume_type is not None and volume_type not in VALID_NOZZLE_VOLUME_TYPES:
