...
class ProbeCommandHelper:
    ...
    def cmd_PROBE_ACCURACY(self, gcmd):
        ...
        fo_params['SAMPLES'] = '1'
        # A single discarded sample is handled below, not per sub-probe
        fo_params['DROP_FIRST_RESULT'] = '0'
        gcode = self.printer.lookup_object('gcode')
        fo_gcmd = gcode.create_gcode_command("", "", fo_params)
        # Probe bed sample_count times
        probe_session = self.probe.start_probe_session(fo_gcmd)
        if params.get('drop_first_result', False):
            # Kalico feature: throw away an initial (often biased) sample
            probe_session.run_probe(fo_gcmd)
            probe_session.pull_probed_results()
            pos = toolhead.get_position()
        ...
        # calculate the average delta between successive probes (Kalico)
        avg_delta = 0.
        if len(positions) > 1:
            delta_sum = 0
...
class ProbeSessionHelper:
    def __init__(self, config, mcu_probe):
        ...
        self.samples_retries = config.getint('samples_tolerance_retries', 0,
                                             minval=0)
        # Discard the first probe result of each probing run (Kalico)
    ...
    def get_probe_params(self, gcmd=None):
        ...
        drop_first_result = gcmd.get_int("DROP_FIRST_RESULT",
                                         int(self.drop_first_result),
                                         minval=0, maxval=1)
        return {'probe_speed': probe_speed,
                ...
                'drop_first_result': bool(drop_first_result)}
