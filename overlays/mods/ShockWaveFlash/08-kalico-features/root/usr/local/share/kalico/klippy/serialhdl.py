...
class SerialReader:
    def __init__(self, reactor, warn_prefix="", mcu=None):
        ...
        # Snapmaker nutzt self._mcu, Kalico self.mcu - beide Namen setzen
        self._mcu = self.mcu = mcu
        # Serial port
    ...
    def dump_debug(self):
        ...
        for i in range(scount):
            ...
            out.append(
                "Sent %d %f %f min_t=%f req_t=%f %d: %s"
                % (
                    i,
                    msg.receive_time,
                    msg.sent_time,
    ...
    def handle_output(self, params):
        ...
        if msg.startswith('Timer too close: ') and self._mcu is not None:
            ...
            parsed = dict(item.strip().split('=', 1)
                          for item in content.split(',') if '=' in item)
            if all(k in parsed for k in ('waketime', 'timer_read_time')):
                ...
                logging.info(
                    "{}Timer too close, wsysclock: {}, tsysclock: {}".format(
                        self.warn_prefix, wsysclock, tsysclock))

        if 'Power loss info saved' == msg and self._mcu is not None:
            mcu_index = {'mcu': 0, 'e0': 1, 'e1': 2, 'e2': 3, 'e3': 4}.get(
                self._mcu._name, 255)
            coded = "0003-0522-%4d-0017" % mcu_index
            self._mcu.get_printer().raise_structured_code_exception(
                coded, msg, 0, 0)
