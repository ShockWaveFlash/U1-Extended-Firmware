...
class LIS2DW:
    def __init__(self, config, lis_type):
        ...
        # An already configured spi bus may be reused (the sensor then has
        # no cs_pin of its own) - see [sensor_accelerometer_identify]
    ...
    def set_reg(self, reg, val, minclock=0):
        ...
        if stored_val != val:
            ...
            err_msg = (
                '{"coded": "0003-0522-0000-0009", "oneshot": 1, "msg":"%s"}'
                % msg
            )
    ...
    def check_devid(self):
        if self.lis_type == LIS2DW_TYPE:
            expect_id = LIS2DW_DEV_ID
        else:
            expect_id = LIS3DH_DEV_ID
        return self.read_reg(REG_LIS2DW_WHO_AM_I_ADDR) == expect_id
        ...
        if self.lis_type == LIS2DW_TYPE:
            if dev_id != LIS2DW_DEV_ID:
                msg = (
                    ...
                    "or a faulty lis2dw chip." % (dev_id, LIS2DW_DEV_ID)
                )
                err_msg = (
                    '{"coded": "0003-0522-0000-0010", "oneshot": 1, "msg":"%s"}'
                    % msg
                )
                raise self.printer.command_error(err_msg)
