...
class LEDHelper:
    def __init__(self, config, update_func, led_count=1):
        ...
        wh.register_mux_endpoint(
            "control/led", "led", name, self._handle_control_led
        )

    def _handle_control_led(self, web_request):
        try:
            red = web_request.get_float("red", 0.0)
            green = web_request.get_float("green", 0.0)
            blue = web_request.get_float("blue", 0.0)
            white = web_request.get_float("white", 0.0)
