...
class PIDCalibrate:
    ...
    def cmd_PID_CALIBRATE(self, gcmd):
        ...
        # Snapmaker: PID-Kalibrierung kann je Heizer gesperrt sein
        if (
            hasattr(heater, "allow_pid_calibrate")
            and not heater.allow_pid_calibrate
        ):
            raise gcmd.error(
                "PID calibration is not allowed for heater '%s'"
                % (heater_name,)
            )
