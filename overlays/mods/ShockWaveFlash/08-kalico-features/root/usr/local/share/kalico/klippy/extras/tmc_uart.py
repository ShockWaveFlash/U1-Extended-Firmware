...
class MCU_TMC_uart_bitbang:
    def __init__(
        self, rx_pin_params, tx_pin_params, select_pins_desc, baud_rate=None
    ):
...
def lookup_tmc_uart_bitbang(config, max_addr):
    ...
    uart_reserved_address = config.getint(
        "uart_reserved_address", 0, minval=0, maxval=max_addr
    )
    mcu_uart = rx_pin_params.get("class")
    if mcu_uart is None:
        mcu_uart = MCU_TMC_uart_bitbang(
            rx_pin_params, tx_pin_params, select_pins_desc, uart_baud_rate
        )
...
class MCU_TMC_uart:
    def __init__(self, config, name_to_reg, fields, max_addr, tmc_frequency):
        ...
        (
            ...
            self.addr,
            self.mcu_uart,
            self.uart_reserved_addr,
        ) = lookup_tmc_uart_bitbang(config, max_addr)
