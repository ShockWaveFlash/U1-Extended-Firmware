...
# Merge-Anpassung: aus Kalicos printer.py uebernommen. Wir starten ueber
# diesen klippy.py-Einstiegspunkt, deshalb muss die wait_while-API hier
# liegen - control_mpc, gcode_macro und heaters (TEMPERATURE_WAIT) rufen sie.
...
class Printer:
    ...
    def wait_while(self, condition_cb, error_on_cancel=True, interval=1.0):
        # Wartet, bis der Callback False liefert - oder der Druck abgebrochen
        # bzw. der Drucker heruntergefahren wird.
        gcode_obj = self.lookup_object('gcode')
        counter = gcode_obj.get_interrupt_counter()
        eventtime = self.reactor.monotonic()
        while condition_cb(eventtime):
            if (self.is_shutdown()
                    or counter != gcode_obj.get_interrupt_counter()):
                if error_on_cancel:
    ...
    def _read_config(self):
        ...
        # Merge-Anpassung: Kalicos mcu.py holt sich danger_options per
        # lookup_object (mcu.py:817). Kalicos eigener Einstiegspunkt laedt es
        # in printer.py:303 - dieser hier tat es nicht, also brach JEDER Start
        # mit "Unknown config object 'danger_options'" ab.
        ...
        # Merge-Anpassung: Kalicos check_unused_options hat ein zweites,
        # pflichtiges Argument (configfile.py:534).
        pconfig.check_unused_options(
            config,
            self.lookup_object("danger_options").error_on_unused_config_options,
        )
