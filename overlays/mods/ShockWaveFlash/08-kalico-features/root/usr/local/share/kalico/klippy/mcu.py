...
import chelper, clocksync, msgproto, pins, serialhdl
...
class MCU_endstop:
    ...
    def _build_config(self):
        ...
        self._mcu.add_config_cmd(
            "config_endstop oid=%d pin=%s pull_up=%d is_pulse_gpio=%d"
...
class MCU_pwm:
    ...
    def set_pwm(self, print_time, value, min_time_interval=None):
        ...
        if min_time_interval is not None:
            min_clock_interval = self._mcu.seconds_to_clock(
                min_time_interval
            )
            if (
                clock < self._last_clock
                or (clock - self._last_clock) < min_clock_interval
            ):
...
class MCU_adc:
    ...
    # Kalico/Mainline nennen diese Methode setup_minmax - Alias fuer
    # Module, die den alten Namen verwenden.
    setup_minmax = setup_adc_sample
...
class MCU:
    ...
    def __init__(self, config, clocksync):
        ...
        adc_except_recorder = self._printer.lookup_object(
            "adc_except_recorder", None
        )
        ...
        if self._name == "mcu":
            ...
            gcode.register_command(
                "QUERY_ADC_EXCEPT_RECORDER",
                self.cmd_QUERY_ADC_EXCEPT_RECORDER,
            )
    ...
    def cmd_QUERY_ADC_EXCEPT_RECORDER(self, gcmd):
        ...
        if adc_except_recorder is not None:
            gcmd.respond_info(
                "adc_except_recorder: {}".format(adc_except_recorder)
            )
    ...
    def check_active(self, print_time, eventtime):
        ...
        if index in (2, 3, 4, 5):
            ...
            for check_name in ["e0", "e1", "e2", "e3"]:
                ...
                if pending < 4:
                    all_down = False
            logging.info(
                "MCU timeout check: %s, all_down=%s",
                " ".join(pending_info),
                all_down,
            )
        ...
        coded_message = '{"coded": "%s", "oneshot": %d, "msg":"%s"}' % (
            coded,
            oneshot,
            err_info,
        )
    ...
    def stats(self, eventtime):
        load = (
            "mcu_awake=%.03f mcu_task_avg=%.06f mcu_task_stddev=%.06f"
            " err_len=%d err_dest=%d err_sync=%d err_crc=%d"
            % (
                self._mcu_tick_awake,
                self._mcu_tick_avg,
                self._mcu_tick_stddev,
                self._mcu_err_msg_len,
                self._mcu_err_msg_dest,
                self._mcu_err_msg_sync,
                self._mcu_err_msg_crc,
            )
        )
        stats = " ".join(
            [
