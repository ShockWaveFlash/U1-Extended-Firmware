...
class VirtualSD:
    def __init__(self, config):
        ...
        # External g-code providers (pa_test / ringing_test)
    ...
    # External g-code providers (pa_test / ringing_test).  The SD card
    # provider is this object itself, so that the file handling of the U1
    # (power loss, exclude_object, print_task_config) stays untouched.
    ...
    # Background work timer for external g-code providers
    def gcode_provider_work_handler(self, eventtime):
        logging.info("Starting print from gcode provider: %s",
                     self.gcode_provider.get_name())
        ...
        while not self.must_pause_work:
            ...
            self.current_line = ""
        logging.info("Exiting print from gcode provider")
        self.work_timer = None
        self.cmd_from_sd = False
    ...
    def work_handler(self, eventtime):
        if self.gcode_provider is not None:
            return self.gcode_provider_work_handler(eventtime)
