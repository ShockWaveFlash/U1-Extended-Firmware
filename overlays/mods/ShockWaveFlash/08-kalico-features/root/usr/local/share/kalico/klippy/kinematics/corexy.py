...
class CoreXYKinematics:
    ...
    def set_position(self, newpos, homing_axes):
        # homing_axes may hold axis letters (Kalico/upstream) or indexes
        # (Snapmaker) - accept both
        homed = [
            a if isinstance(a, int) else "xyz".index(a) for a in homing_axes
        ]
            ...
            if move.axes_d[i] and (
                ...
                if self.limits[i][0] > self.limits[i][1]:
                    raise move.move_error(
                        f"Must home {axis_names[i]} axis first"
                    )
                raise move.move_error(
                    f"Move out of range on {axis_names[i]} axis"
                )
